...
v5.0: Ad Load детекция, floating weights для закрытых комментов.
"""
...
# ============================================================================
# КОНСТАНТЫ ДЛЯ AD LOAD ДЕТЕКЦИИ (v5.0)
# ============================================================================

AD_KEYWORDS = [
    # Русские рекламные маркеры
    "реклама", "промо", "партнерский", "спонсор", "партнёрский",
    "подпишись", "подписаться", "подписывайся", "подписывайтесь",
    "читать продолжение", "забрать доступ", "получить доступ",
    "вступить в канал", "переходи по ссылке", "ссылка в описании",
    "ссылка внизу", "залетай", "переходи", "жми на ссылку",
    # Английские рекламные маркеры
    "#ad", "#promo", "#sponsored", "sponsored by",
    "fair launch", "presale", "whitelist", "airdrop",
    "gem", "x100", "to the moon", "pump",
    "buy now", "contract:", "ca:", "ca :",
    # Крипто-шилл маркеры
    "dyor", "nfa", "not financial advice",
    "🚀🚀🚀", "💎💎💎", "🔥🔥🔥",
]

# Домены которые НЕ считаются рекламой (информационные источники)
WHITELIST_DOMAINS = [
    # Соцсети (свой контент)
    "twitter.com", "x.com", "youtube.com", "instagram.com",
    # Новостные источники
    "coindesk.com", "cointelegraph.com", "bloomberg.com",
    "reuters.com", "forbes.com", "decrypt.co",
    # Блокчейн-эксплореры (информация)
    "etherscan.io", "bscscan.com", "solscan.io",
    "polygonscan.com", "arbiscan.io", "basescan.org",
    # Контент-платформы
    "medium.com", "telegra.ph", "mirror.xyz",
    "substack.com", "notion.so", "github.com",
]

# Шилл-сервисы (всегда реклама)
SHILL_SERVICES = [
    "pinksale.finance", "gleam.io", "crew3.xyz", "galxe.com",
    "zealy.io", "questn.com", "layer3.xyz", "quest3.xyz",
    "dextools.io/app", "dexscreener.com",  # Если ссылка на конкретный токен
]
...
def check_is_ad_post(message: Any, channel_username: str = None) -> dict:
    ...
    ref_patterns = [
        r'\?start=', r'\?ref=', r'ref_id=', r'referral=',
        r'utm_source=', r'aff=', r'affiliate='
    ]
    for pattern in ref_patterns:
        if re.search(pattern, text):
            reasons.append(f"referral_link")
            confidence += 0.6
            break

    # 1.3: Ссылки на шилл-сервисы
    for service in SHILL_SERVICES:
        if service in text:
            reasons.append(f"shill_service: {service}")
            confidence += 0.8
            break

    # 1.4: Проверка на whitelist - если только whitelist ссылки, снижаем уверенность
    url_pattern = r'https?://([a-zA-Z0-9.-]+)'
    urls = re.findall(url_pattern, text)
    non_whitelist_urls = []
    for url in urls:
        is_whitelisted = any(domain in url for domain in WHITELIST_DOMAINS)
        if not is_whitelisted and url not in ['t.me']:
            non_whitelist_urls.append(url)

    # ========== УРОВЕНЬ 2: КОНТЕНТНЫЙ АНАЛИЗ ==========

    # 2.1: Рекламные ключевые слова
    ad_keywords_found = []
    for keyword in AD_KEYWORDS:
        if keyword.lower() in text:
            ad_keywords_found.append(keyword)

    if len(ad_keywords_found) >= 2:
        reasons.append(f"ad_keywords: {ad_keywords_found[:3]}")
        confidence += 0.3
    elif len(ad_keywords_found) == 1:
        confidence += 0.1

    # 2.2: Паттерн контракта (крипто-шилл)
    contract_pattern = r'0x[a-fA-F0-9]{40}'
    if re.search(contract_pattern, text):
        reasons.append("contract_address")
        confidence += 0.7

    # ========== УРОВЕНЬ 3: СТРУКТУРНЫЙ АНАЛИЗ ==========

    # 3.1: Inline кнопки с URL = почти всегда реклама
            ...
            for row in markup.inline_keyboard:
                for btn in row:
                    if hasattr(btn, 'url') and btn.url:
                        # Проверяем что URL не ведёт на whitelist
                        btn_url = btn.url.lower()
                        is_whitelisted = any(domain in btn_url for domain in WHITELIST_DOMAINS)
                        if not is_whitelisted:
                            reasons.append("inline_button_url")
                            confidence += 0.4
                            break

    # 3.2: Репост из чужого канала (forward)
    if hasattr(message, 'forward_from_chat') and message.forward_from_chat:
        fwd_chat = message.forward_from_chat
        fwd_username = getattr(fwd_chat, 'username', None)
        if fwd_username and channel_username:
            if fwd_username.lower() != channel_username.lower():
                # Репост из чужого канала - может быть кросс-промо
                # Но не всегда реклама, поэтому низкая уверенность
                reasons.append(f"forward_from: @{fwd_username}")
                confidence += 0.2
    ...
    # Порог для определения рекламы
    is_ad = confidence >= 0.4 or (len(reasons) >= 2 and confidence >= 0.3)
...
def calculate_ad_load(messages: list, channel_username: str = None) -> dict:
    """
    Рассчитывает Ad Load (заспамленность рекламой) канала.
    v5.0: Процент рекламных постов от общего числа.

    Возвращает:
        {
            'ad_load_percent': float,  # % рекламных постов
            'ad_count': int,           # количество рекламных постов
            'total_posts': int,
            'ad_posts': list[dict],    # детали по каждому рекламному посту
            'status': str              # 'clean', 'moderate', 'heavy', 'spam'
        }
    """
    if not messages:
        return {
    ...
    for msg in messages:
        ...
        if not hasattr(msg, 'text') and not hasattr(msg, 'caption'):
            continue

        total_analyzed += 1
        result = check_is_ad_post(msg, channel_username)
    ...
    ad_load_percent = (ad_count / total_analyzed) * 100

    # Определяем статус
    if ad_load_percent < 10:
        status = 'clean'
    elif ad_load_percent < 30:
        status = 'moderate'
    elif ad_load_percent < 50:
        status = 'heavy'
    else:
        status = 'spam'

    return {
        'ad_load_percent': round(ad_load_percent, 1),
        'ad_count': ad_count,
        'total_posts': total_analyzed,
        'ad_posts': ad_posts[:10],  # Только первые 10 для примера
        'status': status
    }
