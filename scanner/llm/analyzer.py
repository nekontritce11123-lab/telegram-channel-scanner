...
# Unified Ollama client utilities
from .client import call_ollama, safe_parse_json, fill_defaults, regex_extract_fields
...
def analyze_brand_safety(messages: list) -> Optional[dict]:
    ...
    response = call_ollama(BRAND_SAFETY_SYSTEM, prompt)
...
def analyze_comments(comments: list, posts: list = None, channel_type: str = "GENERAL") -> Optional[CommentAnalysisResult]:
    ...
    response = call_ollama(COMMENT_ANALYZER_SYSTEM, prompt)
...
def analyze_ad_percentage(messages: list) -> Optional[int]:
    ...
    response = call_ollama(AD_ANALYZER_SYSTEM, prompt)
